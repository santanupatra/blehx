            ...
            <div class="product-detail-text">
                <?php
                  ...
                  elseif($details->cat_name == 'Litecoin')
                      ...
                      $icon = 'fa-cab'; 
                ?>
                <h3> <i class="fa {{$icon}}"></i> {{$details->cat_name}} ( {{$details->name}} ) <span class="pull-right">${{$details->price}}</span> </h3>
                <p class="bg">
                    <span><i class="fa fa-user"></i> {{$details->uname}}</span> 
                    <span><i class="fa fa-envelope-open-o"></i> {{$details->email}}</span>
                    <span> <i class="fa fa-mobile-phone"></i> {{$details->phone_no}} </span>
                </p> 
                <h4>{{$details->hashrate}} {{$details->hashrate_type}}</h4>
              ...
              <h3 class="h4 pay">Payment</h3>
                ...
                <div class="pay-area" id="paypal_box" style="display: none;">
                    ...
                    <form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post" name="membershipform" id="frmPaypal">
                      ...
                      <input type="hidden" name="custom" value="{{$details->id}}">
                      <input type="hidden" name="return" value="{{URL::to('')}}/order/success/{{$details->id}}">
                      <input type="hidden" name="cancel_return" value="{{URL::to('')}}/order/cancel/{{$details->id}}">
                      <input type="submit" name="pay" class="btn bg-warning pay-now-btn" value="submit">
                    </form>
                </div>
                <div class="pay-area" id="stripe_box">
                    {!! Form::open(['url' => 'order/service_stripe_payment', 'method'=>'post', 'id'=>'frmstripe', 'enctype' => 'multipart/form-data']) !!}
                        <input type="hidden" name="currency_code" value="USD">
                        <input type="hidden" name="amount" id="amount" value="{{$details->price}}">
                        <input type="hidden" name="custom" id="custom" value="101">
                        <input type="hidden" name="id" value="{{$details->sp_id}}">
                        <input type="hidden" name="strip_token" id="strip_token_gift" class="form-control total">
                        <input type="hidden" name="cat_name" value="{{$details->cat_name}}">
                        <input type="hidden" name="hashrate" value="{{$details->hashrate}}">
                       <!--  <div class="form-group" style="text-align: center;">
                            <label for="exampleInputEmail1">Total Amount:</label>
                            ${{$details->price}}                            
                        </div> -->
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="pay-card"> <img src="{{ URL::asset('assets/img/visa.jpg') }}" > </div>
                                    <div class="pay-card"> <img src="{{ URL::asset('assets/img/mastercard.jpg') }}"> </div>
                                    <div class="pay-card"><img src="{{ URL::asset('assets/img/amex.jpg') }}"> </div>
                                </div>
                                <div class="col-md-6">                                    
