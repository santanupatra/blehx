...
class CampignsController extends Controller
{
    ...
    public function index(Request $request)
    {
      ...
      if(!empty($query['sort']))
      {
        if($query['sort']=='end_date')
        {
          $dbquery->orderBy($query['sort'], "asc");
        }
        else
        {
      ...
      $count_current_campaign=$dbquery->count();
    ...
    $get_heighest_donated_campaign[0]->username=$get_heighest_donated_campaign[0]->user->first_name.' '.$get_heighest_donated_campaign[0]->user->last_name;
    ...
    $get_heighest_donated_campaign[0]->profile_photo=!empty($get_heighest_donated_campaign[0]->profilephoto->path)?$img_path.pathinfo($get_heighest_donated_campaign[0]->profilephoto->path, PATHINFO_BASENAME):$img_path.'nouser.png';
    ...
    return view('Campaigns/index',["campaigns"=>$campigns,"categories"=>$categories,"query"=>$query,"heighest_donated_campaign"=>$get_heighest_donated_campaign[0],"count_current_campaign"=>$count_current_campaign]);
    }
  ...
  public function details($slug) 
  {
    ...
    return view('Campaigns/details',["campaign"=>$campaign,"favourite"=>$is_favorite,"follow"=>$is_follow]);
  }
  ...
  public function myproject(Request $request)
  {
    ...
    //echo '<pre>'; print_r($campigns); exit;
  ...
  public function favorites()
  {
    ...
    $favorites->setCollection(
      ...
      ->map(function($item, $key)
      {
        ...
        $item->profile_image=!empty($campaign->profilephoto->path)?$img_path.pathinfo($campaign->profilephoto->path, PATHINFO_BASENAME):$img_path.'nouser.png';
  ...
  public function startproject(Request $request) 
  {
    ...
    $img_path=Config::get('constants.IMGPATH');
    $startproject=Start_Project::where("id","=",1)
                ->first();

    $banner_image=Upload::where('id', '=', $startproject->banner_image)->get();
    $startproject['banner_image']=$img_path.pathinfo($banner_image[0]->path, PATHINFO_BASENAME);

    //echo '<pre>'; print_r($startproject);

    $categories= Category::where("parent_id","=",0)->orderBy('name')->get();
    $selectcategory=$request->session()->get('selectcategory');
    return view('Campaigns/startproject',['categories'=>$categories,"selectcategory"=>$selectcategory,'start_project'=>$startproject]);
  }
  ...
  public function editcampaign($slug=null,$tab=null) 
  {
    ...
    return view('Campaigns/create',['categories'=>$categories,"subcategories"=>!empty($subcategories)?$subcategories:array(),"campaign"=>$campaign,'slect_tab'=>!empty($tab)?$tab:'']);
  }
  ...
  public function myactivity()
  {
    ...
    return view('Campaigns/activity',array('backed'=>$backed,'follow'=>$is_follow,'followers'=>$followers));
  }
